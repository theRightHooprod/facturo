	...
	const handleLoadDir = async () => {
		...
		if (result.success) {
			const files: FileObject[] = await Promise.all(
				result.files.map(async (file: FileObject) => {
					if (file.type === 'XML') {
						try {
			...
			const invoices: Invoice[] = xmlFiles!.map((xml: FileObject) => {
				...
				return {
					...
					folio: (xml.contents! as Contents)["cfdi:Comprobante"].attributes.Folio,
					emisor:
						(xml.contents! as Contents)["cfdi:Comprobante"]["cfdi:Emisor"]?.[0].attributes
							.Nombre,
					date: (xml.contents! as Contents)["cfdi:Comprobante"].attributes.Fecha,
					subtotal: (xml.contents! as Contents)["cfdi:Comprobante"].attributes.SubTotal,
					iva: (xml.contents! as Contents)["cfdi:Comprobante"]["cfdi:Conceptos"]?.[0][
						"cfdi:Concepto"
					]?.[0]["cfdi:Impuestos"]?.[0]["cfdi:Traslados"]?.[0][
						"cfdi:Traslado"
					]?.[0].attributes.Importe,
					total: (xml.contents! as Contents)["cfdi:Comprobante"].attributes.Total,
					...
					emisorRfc:
						(xml.contents! as Contents)["cfdi:Comprobante"]["cfdi:Emisor"]?.[0].attributes
							.Rfc,
					notes:
						(xml.contents! as Contents)["cfdi:Comprobante"]["cfdi:Addenda"]?.[0][
